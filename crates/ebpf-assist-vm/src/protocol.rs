...
mod tests {
    ...
    fn test_guest_command_serialization() {
        let cmd = GuestCommand::Load {
            program_bytes: vec![1, 2, 3, 4],
            name: "test".to_string(),
        };
        ...
        match parsed {
            GuestCommand::Load { name, .. } => assert_eq!(name, "test"),
    ...
    fn test_guest_response_serialization() {
        ...
        match parsed {
            GuestResponse::Status { uptime_secs, .. } => assert_eq!(uptime_secs, 100),
            _ => panic!("Wrong variant"),
        }
    }
    ...
    fn test_error_response() {
        let resp = GuestResponse::error("something went wrong");
        assert!(resp.is_error());

        let resp = GuestResponse::Pong;
        assert!(!resp.is_error());
    }
